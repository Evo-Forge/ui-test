...
var render = function ($) {
  ...
  function addRows(evt) {
    if (!$(evt.target).hasClass('selected')) {
      ...
      var getRows = parseInt($(evt.target).html().replace(/[^\d]+/g, '')),
   ...
   */
  function render() {
    var removing = arguments.length <= 0 || arguments[0] === undefined ? false : arguments[0];
...
var filter = function ($) {
  ...
  function filterRows() {
    ...
    var filterValue = this.value.toLowerCase().split(" "),
        filter = false;
    ...
    if (filter) {
      ...
      $tr.filter(function (i, v) {
        var $t = $(this);
        for (var d = 0; d < filterValue.length; ++d) {
      ...
      $('td:visible:containsIN("' + filterValue + '")').each(function () {
        ...
        var selectText = $(this).html().replace(re, function (match) {
...
var select = function ($) {
  ...
  function selectRow(evt) {
    if ($(evt.target).attr('id') !== 'mainCheckbox') {
      ...
      var status = $mainCheckbox.parent().attr('class').trim();
      if (status === 'checkBox') {
